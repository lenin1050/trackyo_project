...
import { Wallet, Sparkles, Check, Sliders, AlertCircle, TrendingDown, DollarSign } from 'lucide-react';
          ...
          <div className="flex-1 space-y-4 max-h-[360px] overflow-y-auto pr-1">
            {loading ? (
              ...
              categoriesMetrics.map((cat) => {
                ...
                return (
                  <div key={cat.category} className="space-y-2 select-none">
                    {/* Headers */}
                    <div className="flex justify-between items-center text-xs font-bold">
                      <span className="text-themeText flex items-center space-x-1.5">
                        <span>{cat.category}</span>
                        {isOverrun && <AlertCircle className="w-4 h-4 text-rose-500 animate-pulse" />}
                      </span>
                      <span className="text-themeTextMuted text-[11px] font-semibold">
                        <span className="font-black text-themeText">₹{cat.spent.toLocaleString('en-IN')}</span>
                        {cat.limit > 0 ? (
                          <>
                            {' '}
                    ...
                    {cat.limit > 0 ? (
                      ...
                      <div className="w-full h-1 bg-themeBorder/50 rounded-full border border-themeBorder/10" />
                    )}
                  </div>
                );
              })
