...
const SMSParserModal = ({ isOpen, onClose, onSave, showToast }) => {
  ...
  const presetTemplates = [
    { name: "Swiggy (Rs.450/Swiggy/HDFC)", text: "Rs.450 spent on Swiggy using HDFC Card." },
    { name: "Netflix (Rs.799/Netflix/SBI)", text: "Debited from SBI A/c: Rs. 799 spent at Netflix Prime." },
    { name: "Amazon (Rs.1200/Amazon/Axis)", text: "Your Axis Bank card was spent with INR 1200 on Amazon Shopping." },
    { name: "Jio (Rs.99/Jio)", text: "Rs.99 recharge on Jio JioFiber was successful." }
  ];
          ...
          <div className="flex flex-col space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {presetTemplates.map((template, i) => (
                <button
                  key={i}
                  onClick={() => setSmsText(template.text)}
          ...
          {parsedData && (
            <div className="border border-themePrimary/40 bg-themePrimary/5 rounded-xl p-4 space-y-3 animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  <p className="text-[10px] text-themeTextMuted uppercase font-bold">Detected Merchant</p>
