...
import { Bell, Check, Trash2, CheckCircle2, AlertTriangle, Sparkles, Clock } from 'lucide-react';

const Notifications = ({ showToast }) => {
  const { notifications, unreadCount, markRead, markAllRead, clearAll, fetchNotifications } = useNotifications();

  // Refresh notifications on mount so newly created backend notifications appear
  useEffect(() => {
    fetchNotifications();
  }, []);
        ...
        <div className="flex space-x-2">
          ...
          {notifications.length > 0 && (
            <button
              ...
              className="py-2 px-4 rounded-xl bg-rose-500/10 hover:bg-rose-500 text-rose-400 hover:text-white transition-all text-xs font-bold flex items-center space-x-1.5 border border-rose-500/20"
            >
      ...
      <GlassCard className="space-y-4">
        {notifications.length > 0 ? (
          ...
          <div className="text-center py-24 text-themeTextMuted flex flex-col items-center">
            ...
            <p className="text-xs font-bold text-themeText">No notifications found.</p>
          </div>
        )}
      </GlassCard>
