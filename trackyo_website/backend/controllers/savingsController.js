...
exports.createSavingsGoal = async (req, res) => {
  try {
    const { goalName, targetAmount, targetDate, imageUrl, title } = req.body;

    const goal = await SavingsGoal.create({
      user: req.user._id,
      goalName: goalName || title,
      targetAmount: Number(targetAmount),
      targetDate: targetDate || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000), // Default 1 year from now
...
exports.updateSavingsGoal = async (req, res) => {
  try {
    ...
    goal.goalName = goalName || title || goal.goalName;
...
exports.depositToGoal = async (req, res) => {
  try {
    ...
    if (Number(amount) > remaining) {
      return res.status(400).json({ success: false, message: `Deposit exceeds remaining target. Remaining is Rs.${remaining}` });
    }
...
// @desc    Withdraw funds from savings goal
...
exports.withdrawFromGoal = async (req, res) => {
  try {
    const { amount } = req.body;
    if (amount === undefined || amount === null || Number(amount) <= 0) {
      return res.status(400).json({ success: false, message: 'Please specify a valid withdrawal amount greater than 0' });
    }
    ...
    if (Number(amount) > goal.currentAmount) {
      return res.status(400).json({ success: false, message: `Cannot withdraw more than the current amount saved of Rs.${goal.currentAmount}` });
    }
    ...
    res.json({ success: true, message: 'Funds withdrawn successfully', data: savedGoal });
  } catch (error) {
    console.error(error);
    res.status(500).json({ success: false, message: error.message });
  }
