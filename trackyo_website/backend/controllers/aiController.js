...
const parseLocalOCR = (ocrText) => {
  ...
  let amount = 0;
  const amountRegexes = [
    /total\s*charged\s*:\s*(?:inr|rs\.?)?\s*(\d+(?:\.\d{1,2})?)/i,
    /total\s*bill\s*paid\s*:\s*(?:inr|rs\.?)?\s*(\d+(?:\.\d{1,2})?)/i,
    /total\s*amount\s*payable\s*:\s*(?:inr|rs\.?)?\s*(\d+(?:\.\d{1,2})?)/i,
    /(?:total|amount|net\s*payable|gpay|upi|paid|cash)\s*[:\-=]?\s*(?:rs\.?|inr)?\s*(\d+(?:\.\d{1,2})?)/i,
...
Format your response as a valid JSON object only. Example: {"amount": 799, "merchantName": "Zomato", "category": "Food", "date": "2026-05-28"}`;
    ...
    console.log('[DEBUG parseOCRText] ocrText input:', JSON.stringify(ocrText));
    console.log('[DEBUG parseOCRText] parsed result:', JSON.stringify(parsed));
    res.json({
      success: true,
      data: parsed,
      source: 'Local Heuristics Engine'
